...
## 4.2 Spanning Tree protocols (STP)
...
- Redundant path: Set up for higher fault tolerance, since if one down can forward frame to another switch. Usual setup for this is with 2 switches and 2 hubs

- STP Overview:
    - Enterprise switches has STP and switch loops are prevented
    ...
    - 2 different STP type/ variants:
        - Original STP (Defined as IEEE 802.1d)
            - Loop-free topology in network w/ redundant links
            - Speed: Slower
...
- VERY VERY IMPORTANT STP Operations
    ...
    - Determining Root Bridge, Root Port, Designated port !
        - Step 1: How to find Root Bridge? 
            ...
            - Please locate Bridge ID with `sh version` or `sh spanning-tree` on privileged level
        ...
        - Step 2: How to find the (already forwarding state) Root ports in the non-root bridges
        ...
        - Step 3: How to find designated port (default as forwarding state) between segments!
            - If there are 6 segments, then there needs to be 6 designated ports (?)
            - Method: There are 4 steps
    ...
    - Summarizing RSTP switching ports
        ...
        - Backup port (a special block port) (Rarely ask in CCNA, more in CCNP)
            - Will be put to discarding state hmm...
...
- Open switch3 at privileged mode with `sh spanning-tree`, it shows general STP info and roles/ states of the ports in the swtich
    ...
    ```
- Please recall Switch 2 is elected Root bridge due to it's lowest Bridge ID (i.e. actually the lowest MAC address :D)
